...
{
    protected array|int|bool $allowAnonymous = ["save-vote-async", "save-vote"];
    ...
    {
        $contestItems = \therefinery\craftcmscontests\CraftcmsContests::getInstance()->contestService->getAllContests();
    ...
    public function actionSaveVoteAsync(): Response
    {
        ...
        if (array_key_exists("csrfToken", $data)) {
            ...
            $unmaskedRequestToken = Craft::$app
                ->getSecurity()
                ->unmaskToken($requestToken);

            if (
                $unmaskedRequestToken !==
                Craft::$app
                    ->getSecurity()
                    ->unmaskToken(Craft::$app->request->csrfToken)
            ) {
                // If the token is supplied but is invalid, as in possibly being forged or is stale...
                Craft::warning(
                    "ERROR: incoming async vote payload has invalid CSRF token.",
                    "craft-cms-contests",
                );
                return $this->asJson([
                    "status" => "error",
                    "errors" => [
                        [
                            "detail" =>
                                "Invalid CSRF token supplied. Please refresh and try again.",
                        ],
                    ],
                ])->setStatusCode(400);
            }
            ...
            Craft::warning(
                "WARNING: incoming async vote payload does not have CSRF token.",
                "craft-cms-contests",
            );
            return $this->asJson([
                "status" => "error",
                "errors" => [
                    [
                        "status" => "error",
                        "errors" => [["detail" => "CSRF token not supplied."]],
                    ],
                ],
            ])->setStatusCode(400);
        }

        $contest = \therefinery\craftcmscontests\CraftcmsContests::getInstance()->contestService->getContestById(
            $data["contestId"],
        );

        if (!$contest) {
            Craft::warning(
                "WARNING: incoming async vote payload does not have a valid contest.",
                "craft-cms-contests",
            );
            return $this->asJson([
                "status" => "error",
        ...
        if (!empty($contest->recaptchaSecret)) {
            ...
            if (array_key_exists("recaptchaResponse", $data)) {
                ...
                $response = \therefinery\craftcmscontests\CraftcmsContests::getInstance()->contestService->validateRecaptcha(
                    $contest,
                    $ipAddress,
                ...
                if (!$response->isSuccess()) {
                    ...
                    Craft::warning(
                        "WARNING: Failure reCaptcha response coming back from Google on contest id={$contest->id}. Error codes: {$errorCodes}",
                        "craft-cms-contests",
                    );
                    return $this->asJson([
                        "status" => "error",
                        "errors" => [
                            [
                                "detail" =>
                                    "There were some reCaptcha issues. Please try again soon.",
                            ],
                        ],
                    ])->setStatusCode(400);
                }
            } else {
                // recaptcha is set up and response is not in the payload, this is an error
                Craft::warning(
                    "WARNING: incoming vote does not have a recaptchaResponse in data, but recaptcha is enabled on contest id={$contest->id}.",
                    "craft-cms-contests",
                );
                return $this->asJson([
                    "status" => "error",
        ...
        foreach ($data["data"] as $voteData) {
            ...
            $saveVote = \therefinery\craftcmscontests\CraftcmsContests::getInstance()->contestVoteService->saveVote(
                $vote,
                $contest,
            );

            if (!$saveVote["success"]) {
                array_push($errors, ["detail" => $saveVote["message"]]);
            }
        }
    ...
    public function actionSaveVote(): void
    {
    }

    public function actionVotesByContest($contestId = null): Response
    {
        ...
        $voteCounts = \therefinery\craftcmscontests\CraftcmsContests::getInstance()->contestVoteService->getAllVoteCountsByContestId(
            $contestId,
        );

        $allEntries = \therefinery\craftcmscontests\CraftcmsContests::getInstance()->contestService->getAllEntriesByContestId(
            $contestId,
        );
        ...
        foreach ($allEntries as $entry) {
            array_push($voteCountsWithEntries, [
        ...
        // $voteCountsWithEntries[0] = number of votes
        // $voteCountsWithEntries[1] = Craft entry
        usort($voteCountsWithEntries, function ($a, $b) {
            if ($a[0] == $b[0]) {
                return 0;
            }
            return $a[0] < $b[0] ? 1 : -1;
        ...
        return $this->renderTemplate(
            "craft-cms-contests/votes/votesByContest",
            $variables,
        );
    }
}
